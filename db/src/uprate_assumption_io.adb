--
-- Created by ada_generator.py on 2012-07-24 19:03:55.715710
-- 
with Wsc_Db_Data;
...
package body Uprate_Assumption_IO is
   ...
   --
   function Retrieve_By_PK( Username : Unbounded_String; Run_Id : integer; Target : uprate_assumption_target_Enum ) return Wsc_Db_Data.Uprate_Assumption is
   ...
   function Retrieve( sqlstr : String ) return Wsc_Db_Data.Uprate_Assumption_List.Vector is
      ...
      Percent_Change_len : aliased SQLINTEGER := Percent_Change'Size/8;
      Use_Obr_len : aliased SQLINTEGER := Use_Obr'Size/8;
      Target_len : aliased SQLINTEGER := Target'Size/8;
      Element_len : aliased SQLINTEGER := Element'Size/8;
      Uprate_Assumption : Wsc_Db_Data.Uprate_Assumption;
   begin
      ...
      begin -- exception block
         ...
         loop
            ...
            Uprate_Assumption.Target := uprate_assumption_target_Enum'Val( Target );
            Uprate_Assumption.Element := uprate_assumption_element_Enum'Val( Element );
   ...
   procedure Add_Target( c : in out d.Criteria; Target : uprate_assumption_target_Enum; op : d.operation_type:= d.eq; join : d.join_type := d.join_and ) is   
   elem : d.Criterion := d.make_Criterion_Element( "target", op, join, Integer( uprate_assumption_target_Enum'Pos( Target )) );
   begin
      d.add_to_criteria( c, elem );
   end Add_Target;


   procedure Add_Element( c : in out d.Criteria; Element : uprate_assumption_element_Enum; op : d.operation_type:= d.eq; join : d.join_type := d.join_and ) is   
   elem : d.Criterion := d.make_Criterion_Element( "element", op, join, Integer( uprate_assumption_element_Enum'Pos( Element )) );
   begin
      d.add_to_criteria( c, elem );
   end Add_Element;
