...
package body Model.WSC.Household.Transitions_Basic is
   ...
   OVER_50s_Only : constant Boolean := True;
...
-- 2025
   ...
   begin
      for i in 1 .. Full_Weights_Vector'Length loop
         case i is 
            when 1 .. 5 => comp( 1 ) := comp( 1 ) + full( i );
            when 6 .. 15 => comp( i - 4 ) := full( i );
            when 16 .. 20 => comp( 12 ) := comp( 12 ) + full( i );
            when 21 .. Full_Weights_Vector'Last => comp( i - 8 ) := full( i );
         end case;
      end loop;
      return comp;
   ...
   begin
      ...
      Assert( s > 0.0, "S populations is all zeros for hh " & hh.hid'Img );
      return obs;
   ...
   begin
      ...
      declare
         ...
         weight              : Amount;
         ct                  : Row_Vector;
         compressed_targets  : constant Row_Vector := Compress_Full_Vector( target_populations );
      begin
         ...
         for hhno in 1 .. num_households loop
            ...
            -- weight := 1.0;
            ct := Make_Vector( hh );
            Assert( Sum( ct( Short_Populations_Range )) > 0.0, " populations all zero for hh " & hh.hid'Img );
         ...
         Log( "Targets " & To_String( compressed_targets ));
         Do_Reweighting(
            Data               => obs, 
            Which_Function     => wsc_run.Weighting_Function,
            Initial_Weights    => initial_weights,            
            Target_Populations => compressed_targets,
            TolX               => 0.01,
            TolF               => 0.01,
            Max_Iterations     => 40,
      ...
      for wave in Estimated_Data_Waves loop
         ...
         declare
            ...
            package Reweigher is new Maths_Funcs.Weights_Generator(    
               ...
               Num_Observations  => num_households );
            use Reweigher;
            data                : Reweigher.Dataset := ( others => ( others => 0.0 ));
            full_row            : Full_Weights_Vector;
            initial_weights     : Col_Vector;
            weights             : Col_Vector;
