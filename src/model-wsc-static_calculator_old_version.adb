...
package body Model.WSC.Static_Calculator is
   ...
   --
   -- FIXME move this to Data Creation routine
   --
   procedure Calculate_DLA_From_Actual_Data( 
      pers            : Person; 
      care            : out Amount;
      mobility        : out Amount;
   ...
   begin
      care := 0.0;
      mobility := 0.0;
      if( pers.annual_income( disab_liv_allowcare ) > 0.0 )then
         if( pers.current_income( disab_liv_allowcare ) > 60.0 )then -- BHPS AA/DLA amounts are all over the place; guess that
            -- this is higher amount
            care := sys.care.benefit_rate( high );
         elsif( pers.current_income( disab_liv_allowcare ) > 40.0 ) then
            care := sys.care.benefit_rate( middle );
         else
            care := sys.care.benefit_rate( low );
         end if;
      end if;
      if( pers.annual_income( disab_liv_allowmob ) > 0.0 )then
         if( pers.current_income( disab_liv_allowmob ) > 60.0 )then -- BHPS AA/DLA amounts are all over the place; guess that
            -- this is higher amount
            mobility := sys.mobility.benefit_rate( high );
         else
            mobility := sys.mobility.benefit_rate( low );
         end if;
      end if;
      if( pers.annual_income( disab_liv_allow_dk ) > 0.0 )then
         if( mobility > 0.0 ) and ( care = 0.0 )then
            if( pers.current_income( disab_liv_allow_dk ) > 60.0 )then -- BHPS AA/DLA amounts are all over the place; guess that
               -- this is higher amount
               care := sys.care.benefit_rate( high );
            elsif( pers.current_income( disab_liv_allow_dk ) > 40.0 ) then
               care := sys.care.benefit_rate( middle );
            else
               care := sys.care.benefit_rate( low );
            end if;
         elsif( mobility > 0.0 ) and ( care = 0.0 )then
            if( pers.current_income( disab_liv_allow_dk ) > 60.0 )then -- BHPS AA/DLA amounts are all over the place; guess that
               -- this is higher amount
               mobility := sys.mobility.benefit_rate( high );
            else
   ...
   end Calculate_DLA_From_Actual_Data;
   ...
   begin
      ...
      income := Calculate_Income( bu=>bu, res=>res, which_to_include=>gpcsys.incomes, use_current=>True );
   ...
   begin
      ...
      qualifying_income := Calculate_Income( bu, res, sys.qualifying_incomes, True );
         ...
         case prs.passes_non_residential_means_test is
            when not_entitled =>
               ...
               begin
                  ...
                  Log( "Calculate_Non_Residential; got probit v as " & Format( pr ) & " rand " & Format( rand ) & " demands_private_care " & demands_private_care'Img & " hours offerered " & Format( latent_hours_of_care_demand ));
   ...
   begin
         ...
         begin
            ...
            for adno in  1 .. bu.Num_Adults loop
               ...
               begin
                  prs.sernum := ad.pid;
                  prs.is_residential := ad.years_in_residential_care > 0;
                  ad_last_period := db.Get_Person(
                     Waves'Pred( wave ),
                     ad.pid );
                  if( ad.pid = ad_last_period.pid )then
                     null;
                  end if;
                  -- we fill calculated with recorded
                  -- gross incomes and overwrite AA and DLA
                  ...
                  Calculate_Attendance_Allowance_From_Actual_Data(
                        ad,
                        prs.income( attendance_allow ),
                        0.0, 
                        params.benefits.attendance_allowance,
                  ...
                  Calculate_DLA_From_Actual_Data( 
                     ad,
                     prs.income( disab_liv_allowcare ),
                     prs.income( disab_liv_allowmob ),
                     0.0,
      ...
      Accumulate( hh, results, wsc_run);
   end Calculate_One_Household_With_No_History;
