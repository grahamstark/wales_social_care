--
-- Created by ada_generator.py on 2012-02-09 16:45:16.095815
-- 
with Ada.Calendar;
...
package body DB_Commons is
   ...
   function Load_Postgres( filename : String; results_file : String ) return Boolean is
   use System;
   use Gnat.OS_Lib;
      ...
      pid     : Integer;
   begin
      args( 1 ) := new String'( Environment.Get_Server_Name );
      args( 2 ) := new String'( "-U" );
      args( 3 ) := new String'( Environment.Get_Username );
      args( 4 ) := new String'( "-f" );
      args( 5 ) := new String'( filename );
      Spawn(
         Program_Name => "/usr/bin/psql", 
         Args         => args, 
   ...
   function To_String( op : Operation_Type ) return String is
   begin
      return likeStr( op );
   end To_String;
