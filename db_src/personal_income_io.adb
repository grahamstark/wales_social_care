--
-- Created by ada_generator.py on 2012-02-10 02:09:15.593259
-- 
with Wsc_Db_Data;
...
package body Personal_Income_IO is
   ...
   --
   log_trace : GNATColl.Traces.Trace_Handle := GNATColl.Traces.Create( "PERSONAL_INCOME_IO" );
   ...
   --
   function Retrieve_By_PK( Username : Unbounded_String; Run_Id : integer; Pid : Big_Integer; Sysno : integer; Iteration : integer; Wave : Unbounded_String; Income_Type : Incomes_Type ) return Wsc_Db_Data.Personal_Income is
   ...
   --
   function Highest_Values_Of( sqlstr : String ) return Wsc_Db_Data.Personal_Income_List.Vector is
      type Timestamp_Access is access all SQL_TIMESTAMP_STRUCT;
      type Real_Access is access all Real;
      type String_Access is access all String;
      LOC_SELECT_PART : constant String := "select " &
         " income_type, " &
        " case when max( value ) is null then 0.0 else max( value ) end as max_value " &
         "from personal_income where " ;

      l : Wsc_Db_Data.Personal_Income_List.Vector;
      ps : SQLHDBC := SQL_NULL_HANDLE;
      has_data : Boolean := false;
      connection : dodbc.Database_Connection := Connection_Pool.Lease;
      query : constant String := LOC_SELECT_PART & " " & sqlstr & " group by income_type ";
      --
      ...
      Income_Type_len : aliased SQLLEN := Income_Type'Size/8;
      Value_len : aliased SQLLEN := Value'Size/8;
      --
   ...
   begin
      ...
      begin -- exception block
         ...
         loop
            ...
            Personal_Income.Income_Type := Incomes_Type'Val( Income_Type );
            Personal_Income.Value:= Real( Value_access.all );
            l.Append( Personal_Income );
         end loop;
         Log( "retrieve: Query Run OK; got " & l.Length'Img & " items " );
      exception 
         when No_Data => Null; 
         when Error : others =>
            Log( "exception raised " & Exception_Information(Error) );
            Raise_Exception( d.DB_Exception'Identity, 
      ...
      exception 
         when Error : others =>
            Log( "return to pool exception raised " & Exception_Information(Error) );
            Raise_Exception( d.DB_Exception'Identity, 
               "retrieve: exception " & Exception_Information(Error) );
      end; -- exception block
      Log( "returning safely" );
      return l;
   ...
   procedure Add_Income_Type( c : in out d.Criteria; Income_Type : Incomes_Type; op : d.operation_type:= d.eq; join : d.join_type := d.join_and ) is   
   elem : d.Criterion := d.make_Criterion_Element( "income_type", op, join, Integer( Incomes_Type'Pos( Income_Type )) );
   begin
      d.add_to_criteria( c, elem );
   end Add_Income_Type;
