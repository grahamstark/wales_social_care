...
package body Parameter_System.Input_Buffer.WSC_Renderer is
   ...
   package wscs renames Model.WSC.Global_Settings; 
   ...
   log_trace : GNATColl.Traces.Trace_Handle := GNATColl.Traces.Create( "PARAMETER_SYSTEM.INPUT_BUFFER.WSC_RENDERER" );
   ...
   function Make_Parameter_Menu(
      ...
      prefix               : Unbounded_String;
      lang                 : Languages;
      user                 : Model.WSC.Users.User_Type ) return Unbounded_String is  
   ...
   begin
      ...
      Make_Input_Menus( base_sys, locpath, lists, user, lang, depth, full_prefix );
   ...
   begin
      ...
      Insert( translations, Assoc( "ROOT", wscs.WSC_Web_Root ));
      ...
      return Web_Utils.Parse_Template( 
         TuS( wscs.template_components_path ) &
         wscs.Dir_Separator & "jquery_help", translations );
   end Write_Help_Box;
  ...
  function Make_Ajax_Call_Indexed( 
      ...
      ERROR_CHECK_JS : constant String := " new Ajax.Updater( 'error_section', '" & wscs.WSC_Web_Root & "ajax', { method: 'post', parameters: 'action=error_check'})";
   use Templates_Parser;
      translations       : Translate_Set;
   ...
   begin
      case action is
      when insert_above =>
         Insert( translations, Assoc( "IMG", ROOT & INSERT_BEFORE_IMG ));
         Insert( translations, Assoc( "ALT", Lookup( "ajax_insert_before",lang )));
      when insert_below => 
         Insert( translations, Assoc( "IMG", ROOT& INSERT_AFTER_IMG ));
         Insert( translations, Assoc( "ALT",  Lookup( "ajax_insert_after",lang )));
      when delete => 
         Insert( translations, Assoc( "IMG", ROOT& DELETE_IMG ));
         Insert( translations, Assoc( "ALT", Lookup( "ajax_delete",lang )));
      when save => 
         Insert( translations, Assoc( "IMG", ROOT& SAVE_IMG ));
         Insert( translations, Assoc( "ALT", Lookup( "ajax_save",lang )));
      when copy =>
         Insert( translations, Assoc( "IMG", ROOT& COPY_IMG ));
      ...
      Insert( translations, Assoc( "URL", ROOT & "array_update" ));
      Insert( translations, Assoc( "TABLE-CONTAINER-ID", ajax_target_key ));
      Log( "path for templates |" & wscs.template_components_path & "/jquery_ajax" & ".thtml |" );
      return Web_Utils.Parse_Template( TuS( wscs.template_components_path ) & 
         wscs.Dir_Separator &
   ...
   begin
      ...
      return Web_Utils.Parse_Template( TuS( wscs.template_components_path ) & 
         wscs.Dir_Separator & "static_input_one_year", translations );               
   ...
   begin
      return Web_Utils.Parse_Template( TuS( wscs.template_components_path ) & 
         wscs.Dir_Separator & "input", translations );      
